...
interface LogItemProps {
	...
	// Local filter state (filter UI removed; state kept for backwards compat)
...
export const TerminalOutput = memo(
	forwardRef<HTMLDivElement, TerminalOutputProps>((props, ref) => {
		...
		const _toggleLocalFilter = useCallback((logId: string) => {
		...
		// Callback to update filter mode for a log entry (kept for backwards compat)
		const _setFilterModeForLog = useCallback(
			(
				logId: string,
		...
		// Callback to clear local filter for a log entry (kept for backwards compat)
		const _clearLocalFilter = useCallback(
			(logId: string) => {
