...
function MaestroConsoleInner() {
	...
	const restoreSession = async (session: Session): Promise<Session> => {
		try {
			...
			// Migration: ensure terminal tabs exist (for sessions created before terminal tabs support)
			...
			if (aiSuccess) {
				...
				// Keeping field for session persistence backwards compatibility
				...
				try {
					...
					for (const subdir of gitSubdirs) {
						...
						const worktreeSession: Session = {
							...
							aiLogs: [],
							shellLogs: [], // DEPRECATED: terminal uses xterm.js now
							workLog: [],
							contextUsage: 0,
	...
	useEffect(() => {
		const handleRemoteCommand = async (event: Event) => {
			...
			if (effectiveInputMode === 'terminal') {
				...
				// Set state to busy (shellLogs mutation removed - terminal uses xterm.js now)
				setSessions((prev) =>
					prev.map((s) => {
						if (s.id !== sessionId) return s;
						return {
				...
				try {
					...
					// shellLogs error mutation removed - terminal uses xterm.js now
					setSessions((prev) =>
						prev.map((s) => {
							if (s.id !== sessionId) return s;
							return {
				...
				try {
					...
					setSessions((prev) =>
						prev.map((s) => {
							...
							// DEPRECATED: shellLogs kill log removed - terminal uses xterm.js now
					...
					setSessions((prev) =>
						prev.map((s) => {
							...
							// DEPRECATED: shellLogs error log removed - terminal uses xterm.js now
							return {
								...s,
								state: 'idle',
								busySource: undefined,
